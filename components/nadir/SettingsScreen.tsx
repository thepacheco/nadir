...
export default function SettingsScreen() {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "28px 32px", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ marginBottom: 24, flex: "none" }}>
      ...
      <div style={{ display: "flex", gap: 24, borderBottom: "1px solid rgba(20,24,28,0.1)", marginBottom: 24, flex: "none" }}>
        {TABS.map(tab => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            style={{
              background: "transparent", border: "none", padding: "0 0 10px 0", cursor: "pointer",
              fontFamily: "inherit", fontSize: 14, fontWeight: activeTab === tab.id ? 600 : 500,
              color: activeTab === tab.id ? "#0E7C8A" : "#7a848e",
              borderBottom: activeTab === tab.id ? "2px solid #0E7C8A" : "2px solid transparent",
              transition: "all 0.2s"
            }}
          >
            {tab.label}
